...
def test_login_notifies_extension_when_ws_connected(client):
    """POST /api/auth/login pushes a `login` message to the extension
    so it can open the Flow tab in the foreground."""
    sent: list[dict] = []
    ...
    assert r.status_code == 200
    assert r.json() == {"ok": True, "extension_notified": True}
