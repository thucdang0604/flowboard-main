...
class FlowClient:
    ...
    async def notify(self, message: dict) -> bool:
        ...
        Used by:
        - logout: tell extension to clear its in-memory token + cached userinfo
        - scan: ask extension to re-fetch userinfo when the agent has a connection but the cache is empty
        - login: ask extension to open the Flow tab for the user to sign in
        """
    ...
    async def login(self) -> bool:
        """Nudge the extension to open the Flow tab for login."""
        return await self.notify({"type": "login"})
