  ...
  ws.onmessage = async ({ data }) => {
    try {
      ...
      if (msg.type === 'callback_secret') {
        ...
        // Agent's /api/auth/login invoked — open the Flow tab in the
        // foreground so the user can sign in.
        console.log('[Flowboard] login requested by agent');
        openFlowTabResilient(true);
